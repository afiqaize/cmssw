...
bool EgammaCutBasedEleId::PassTriggerCuts(TriggerWorkingPoint triggerWorkingPoint, const reco::GsfElectron &ele) {
  ...
  return EgammaCutBasedEleId::PassTriggerCuts(
      triggerWorkingPoint, isEB, pt, dEtaIn, dPhiIn, sigmaIEtaIEta, hoe, trackIso, ecalIso, hcalIso);
}
...
bool EgammaCutBasedEleId::PassEoverPCuts(const reco::GsfElectronRef &ele) { return PassEoverPCuts(*ele); }
...
unsigned int EgammaCutBasedEleId::TestWP(WorkingPoint workingPoint,
  ...
  // get the mask value
  unsigned int mask = EgammaCutBasedEleId::TestWP(workingPoint,
                                                  isEB,
                                                  pt,
                                                  eta,
                                                  dEtaIn,
...
bool EgammaCutBasedEleId::PassTriggerCuts(const TriggerWorkingPoint triggerWorkingPoint,
  ...
  if (triggerWorkingPoint == EgammaCutBasedEleId::TRIGGERTIGHT) {
    ...
    if (hoe > cut_hoe[idx])
      return false;
    if (trackIso / pt > cut_trackIso[idx])
      return false;
    if (ecalIso / pt > cut_ecalIso[idx])
      return false;
    if (hcalIso / pt > cut_hcalIso[idx])
      return false;
...
bool EgammaCutBasedEleId::PassEoverPCuts(const float eta, const float eopin, const float fbrem) {
  if (fbrem > 0.15)
    return true;
  else if (fabs(eta) < 1.0 && eopin > 0.95)
    return true;
  return false;
}
...
unsigned int EgammaCutBasedEleId::TestWP(WorkingPoint workingPoint,
  ...
  if (workingPoint == EgammaCutBasedEleId::VETO) {
    ...
    cut_dPhiIn[0] = 0.800;
    cut_dPhiIn[1] = 0.700;
    cut_sigmaIEtaIEta[0] = 0.010;
    cut_sigmaIEtaIEta[1] = 0.030;
    cut_hoe[0] = 0.150;
    cut_hoe[1] = 999.9;
    cut_ooemoop[0] = 999.9;
    cut_ooemoop[1] = 999.9;
  ...
  float AEff = ElectronEffectiveArea::GetElectronEffectiveArea(
      ElectronEffectiveArea::kEleGammaAndNeutralHadronIso03, eta, EAtarget);
