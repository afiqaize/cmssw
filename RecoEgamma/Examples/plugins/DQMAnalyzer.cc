...
void DQMAnalyzer::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  for (gsfIter = gsfElectrons->begin(); gsfIter != gsfElectrons->end(); gsfIter++) {
    ...
    h_ele_hcalDepth1RecHitSumEt_dr03->Fill(gsfIter->dr03HcalRecHitSumEt(1));
    h_ele_hcalDepth2RecHitSumEt_dr03->Fill(gsfIter->dr03HcalRecHitSumEt(2));
    h_ele_tkSumPt_dr04->Fill(gsfIter->dr04TkSumPt());
    h_ele_ecalRecHitSumEt_dr04->Fill(gsfIter->dr04EcalRecHitSumEt());
    h_ele_hcalDepth1RecHitSumEt_dr04->Fill(gsfIter->dr04HcalRecHitSumEt(1));
    h_ele_hcalDepth2RecHitSumEt_dr04->Fill(gsfIter->dr04HcalRecHitSumEt(2));
  }
...
bool DQMAnalyzer::isolationCut(const reco::GsfElectronCollection::const_iterator &gsfIter, double vertexTIP) {
  ...
  if (gsfIter->dr03TkSumPt() > tkIso03Max_)
    return true;
  if (gsfIter->isEB() && gsfIter->dr03HcalRecHitSumEt(1) > hcalIso03Depth1MaxBarrel_)
    return true;
  if (gsfIter->isEE() && gsfIter->dr03HcalRecHitSumEt(1) > hcalIso03Depth1MaxEndcaps_)
    return true;
  if (gsfIter->isEE() && gsfIter->dr03HcalRecHitSumEt(2) > hcalIso03Depth2MaxEndcaps_)
    return true;
