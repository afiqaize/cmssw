...
void PFPhotonTranslator::createPhotons(reco::VertexCollection &vertexCollection,
  ...
  for (unsigned iphot = 0; iphot < nphot; ++iphot) {
    ...
    for (int id = 0; id < 7; ++id) {
      isolationVariables03.hcalRecHitSumEt[id] = egPhotonRef_[iphot]->hcalRecHitSumEtConeDR03(id + 1);
    }
    ...
    for (int id = 0; id < 7; ++id) {
      isolationVariables04.hcalRecHitSumEt[id] = egPhotonRef_[iphot]->hcalRecHitSumEtConeDR04(id + 1);
    }
